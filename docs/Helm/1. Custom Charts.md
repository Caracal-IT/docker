...
| or   | Or                                                                 |
| not  | Not                                                                |
| fail | To intenionally abort chart execution and provide an error message |

**To Integer**

``` code
{{- if lt (.Capabilities.KubeVersion.Minor | int) 23 -}}
{{- fail "This chart needs ubernetes version 1.23 or higher." -}}
{{- end }}
```
...
spec:
    ...
    {{- end }}
    selector:
        {{- include "myapp.selectLabels" . | nindent4 }}
```

**Set a Value**

This will not work, the ports value is overridden.  
You will need to supply the entire array.

``` code
helm install myapp01 . --dry-run --set "service.ports[0].number=8000"
```
...
spec:
    ...
    {{- fail "Please select another port number than 8080 for the admin port" -}}
    {{- end}}
        - port: {{ .number }}
          protocol: TCP
          targetPort: {{ .targetPort }}
